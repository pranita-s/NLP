...
tree_structure <- Semantics("It is a great university")
#tree_structure <- as.data.frame(tree_structure)
tree_structure1 <- tree_structure[!(tree_structure$dependent %in% stopwords(kind = "en")),]

#tree_structure$dep_POS <- paste(tree_structure$dependent,tree_structure$POS,sep="~")
"Google operates what is surely the largest computer network on Earth, a system that comprises custom-built,warehouse-sized data centers spanning 15 locations in four continents.But about six years ago, as the company embraced a new form of voice recognition on Android phones,its engineers worried that this network was not nearly big enough."

#If each of the world's Android phones used the new Google voice search for just three minutes a day, these engineers realized,
#the company would need twice as many data centers. "
...
library(StanfordCoreNLP)
p <- StanfordCoreNLP_Pipeline(annotators = "dcoref")
x = '"I voted for Nadar because he was most aligned with my values", she said'
y <- p(x)

ptexts <- sapply(subset(y, type == "document")$features, `[[`, "coreferences")
